...
logpath=$casePath
headerText="run_parallel_rhoCfdemSolverPimple_ErgunTestMPI_CFDDEM"
...
if [ $runOctave == "true" ]
    then
        ...
        #- rmove old graph
        rm rhoCfdemSolverPimple_ErgunTestMPI.png

        #- run octave
        octave --no-gui totalPressureDrop.m

        #- show plot 
        eog rhoCfdemSolverPimple_ErgunTestMPI.png

        #- copy log file to test harness
        cp ../../$logfileName $testHarnessPath
        cp rhoCfdemSolverPimple_ErgunTestMPI.png $testHarnessPath
fi
